    ...
    <div class="card ">
        <div class="card-header px-3 d-flex flex-wrap justify-content-between">
            <div class="d-flex flex-wrap">
                ...
                <h1 class="card-title">{{ capitalizeFirstLetter(analysisType) }} risk
                </h1>
            </div>
            <b-button size="sm" @click="showModal=true">
                <b-icon-info-circle v-b-modal="'modal-info-' + setType" v-b-tooltip:hover.top="'Info'"
                                    class="text-muted"/>
            </b-button>
        ...
        <div class="card-body d-flex flex-column justify-content-center align-items-center h-100 px-0 pt-0 pb-3">
            <div class="w-100 h-100" :class="dimmerClass">
                ...
                <div v-if="initialDataLoaded" style="height: 240px" class="dimmer-content">
                    ...
                    <aggregation-line-chart ref="dayChart" v-if="bucketSize === '24H'" :unit="'minute'"
...
export default class NetworkAnalysis extends Mixins(IsLoadingMixin, StoreMixin) {
    ...
    getAggregatedData(statisticsAggregation: NetworkStatisticsAggregation[], prop: string) {
    ...
    updateAggregatedDataInDataSets(dataSets: ChartDataSets[], statisticsAggregation: NetworkStatisticsAggregation[]) {
        dataSets.forEach(dataSet => {
            switch (dataSet.label) {
                case 'Organization':
                    dataSet.data = this.getAggregatedData(statisticsAggregation, 'min' + this.capitalizeFirstLetter(this.setType) + 'SetOrgs' + (this.canBeFiltered ? 'Filtered' : '') + 'Average');
                    break;
    ...
    getAggregatedLabels(tooltipItem: ChartTooltipItem, data: ChartData) {
        //@ts-ignore
        let avg = data.datasets![tooltipItem.datasetIndex]!.data![tooltipItem.index!]!.y;
        //@ts-ignore
        let min = data.datasets![tooltipItem.datasetIndex + 1]!.data![tooltipItem.index!]!.y;
        //@ts-ignore
        let max = data.datasets![tooltipItem.datasetIndex + 2]!.data![tooltipItem.index!]!.y;
    ...
    async updateYearChart() {
        ...
        try {
            ...
            this.updateAggregatedDataInDataSets(this.aggregatedDataSets, this.yearStatistics);
        } catch (e) {
            console.log(e);
            this.failed = true;
        }
    ...
    async updateDays30Chart() {
        ...
        try {
            ...
            this.updateAggregatedDataInDataSets(this.aggregatedDataSets, this.days30Statistics);
